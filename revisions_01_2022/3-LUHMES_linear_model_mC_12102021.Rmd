...
Gene_CpG_Relations_update <- master_anno_en
...
#[1] "Enrichment: 1; Depletion 0.009; Feature: 3'UTR"
...
#[1] "Enrichment: 0; Depletion 1; Feature: TSS1500"
...
CGI_Gene_permutation_enrichment_enhancer(CpG_list, background.probes, permutation_number, plotmin, plotmax)
#[1] "FDR Adjusted Permutation P values for enrichment and depletion"
#[1] "Enrichment: 0; Depletion 1; Feature: "
#[1] "Enrichment: 1; Depletion 1; Feature: 1stExon"
...
nrow(WT_gene_list[-which(WT_gene_list$Name %in% master_anno_en$site),]) #0, no outliers

#getting the number of hits in each genomic context for WT
locations <- master_anno_en[master_anno_en$site %in% WT_gene_list[WT_gene_list$threshold==TRUE,"Name"],"group"]
loc_summary <- summary(as.factor(locations))
loc_summary
#          1stExon    3'UTR    5'UTR     Body Enhancer  ExonBnd  TSS1500   TSS200 
#    9116      259      428     2006     7595     6100      112     2607     1110 
...
nrow(A30P_gene_list[-which(A30P_gene_list$Name %in% master_anno_en$site),]) #0 - no outliers

locations <- master_anno_en[master_anno_en$site %in% A30P_gene_list[A30P_gene_list$threshold==TRUE,"Name"],"group"]
loc_summary <- summary(as.factor(locations))
loc_summary
#          1stExon    3'UTR    5'UTR     Body Enhancer  ExonBnd  TSS1500   TSS200 
#    1975       58      114      479     1755     1317       15      523      293 
...
#A30P
#intergenic ***, enhancer ***, exonbnd ***, TSS200 ***

#WT
#intergenic ***, 3'UTR **, 5' UTR ***, body ***, enhancer ***, exonbnd ***, TSS1500 ***, TSS200 ***

locations_norm$sig <- as.factor(c("***","","**",rep("***",6),"***",rep("",4),rep("***",2),"","***"))

ggplot(locations_norm, aes(x=Name, y=fold_change, fill=Genotype)) + geom_bar(stat="identity", position="dodge") + theme_bw() + ylab("Fold Change over Array Background") + xlab("Genomic Context") + scale_fill_manual(values=c("gray75", "gray48"), name="Comparison", labels=c("Control vs. WT aSyn", "Control vs. A30P aSyn")) + geom_hline(yintercept=0) + ylim(c(-4,1.5)) + theme(panel.grid.major.x = element_blank(), axis.text.x = element_text(angle=45, hjust=1, size=12), axis.text.y=element_text(size=12))  + 
...
```
![Genomic context fold enrichment for both genotypes - updated](/home/BCRICWH.LAN/sschaffner/KoborLab/kobor_space/sschaffner/LUHMES_201806/clean_scripts/genomic_context_mC_122021.png)
